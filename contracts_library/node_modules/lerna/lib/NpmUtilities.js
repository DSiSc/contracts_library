  ...
  _createClass(NpmUtilities, null, [{
    ...
    value: function installInDir(directory, dependencies, config, callback) {
      ...
      _FileSystemUtilities2.default.rename(packageJson, packageJsonBkp, function (err) {
        ...
        var done = function done(finalError) {
          cleanup();
          unregister();
          callback(finalError);
        };
    ...
    key: "runScriptInDir",
    value: function runScriptInDir(script, _ref2, callback) {
      var args = _ref2.args,
          directory = _ref2.directory,
          npmClient = _ref2.npmClient;
      ...
      _ChildProcessUtilities2.default.exec(npmClient, ["run", script].concat(_toConsumableArray(args)), opts, callback);
    }
  }, {
    key: "runScriptInDirSync",
    value: function runScriptInDirSync(script, _ref3, callback) {
      var args = _ref3.args,
          directory = _ref3.directory,
          npmClient = _ref3.npmClient;

      _npmlog2.default.silly("runScriptInDirSync", script, args, _path2.default.basename(directory));

      var opts = NpmUtilities.getExecOpts(directory);
      _ChildProcessUtilities2.default.execSync(npmClient, ["run", script].concat(_toConsumableArray(args)), opts, callback);
    }
  }, {
    key: "runScriptInPackageStreaming",
    value: function runScriptInPackageStreaming(script, _ref4, callback) {
      var args = _ref4.args,
          pkg = _ref4.pkg,
          npmClient = _ref4.npmClient;

      _npmlog2.default.silly("runScriptInPackageStreaming", [script, args, pkg.name]);

      var opts = NpmUtilities.getExecOpts(pkg.location);
      _ChildProcessUtilities2.default.spawnStreaming(npmClient, ["run", script].concat(_toConsumableArray(args)), opts, pkg.name, callback);
    }
  }, {
    key: "publishTaggedInDir",
    value: function publishTaggedInDir(tag, pkg, _ref5, callback) {
      var npmClient = _ref5.npmClient,
          registry = _ref5.registry;

      var directory = pkg.location;

      _npmlog2.default.silly("publishTaggedInDir", tag, _path2.default.basename(directory));

      var opts = NpmUtilities.getExecOpts(directory, registry);
      var args = ["publish", "--tag", tag.trim()];
      ...
      _ChildProcessUtilities2.default.exec(npmClient, args, opts, callback);
    }
  }, {
    key: "getExecOpts",
